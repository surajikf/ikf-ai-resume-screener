/**
 * Script to save WhatsApp default credentials to Supabase
 * Run: node save-whatsapp-defaults.js
 */
...
async function saveDefaults() {
  console.log('💾 Saving WhatsApp default credentials to Supabase...\n');
  
  const results = [];
  let allSuccess = true;
  
  for (const [key, value] of Object.entries(whatsappDefaults)) {
    try {
  ...
  console.log('\n' + '='.repeat(60));
  if (allSuccess) {
    console.log('✅ All WhatsApp credentials saved successfully!\n');
    console.log('📋 Next steps:');
    console.log('   1. Restart your dev server: npm run dev');
    console.log('   2. Go to http://localhost:3001/settings');
    console.log('   3. You should see all WhatsApp fields pre-filled');
